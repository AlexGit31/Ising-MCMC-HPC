...
static int sommeVoisins(struct grille *grille, int i, int j, int taille) {
    int somme = 0;
    if (i > 0)          somme += grille->sigma[(i - 1) * taille + j]; // Haut
    if (i < taille - 1) somme += grille->sigma[(i + 1) * taille + j]; // Bas
    if (j > 0)          somme += grille->sigma[i * taille + (j - 1)]; // Gauche
    if (j < taille - 1) somme += grille->sigma[i * taille + (j + 1)]; // Droite
    return somme;
}
...
int calculVariationVoisin(struct grille *grille, int i, int j) {
    ...
    int s_i = grille->sigma[i * taille + j];
...
int calculEnergieInitiale(struct grille *grille) {
    ...
    for (int i = 0; i < taille; i++) {
        for (int j = 0; j < taille; j++) {
            ...
            if (j < taille - 1) {
                H -= s_i * grille->sigma[i * taille + (j + 1)]; // Droite
            }
            if (i < taille - 1) {
                H -= s_i * grille->sigma[(i + 1) * taille + j]; // Bas
            }
        }
    }
    return H;
